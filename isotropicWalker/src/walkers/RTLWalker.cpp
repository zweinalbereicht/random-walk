...
double  RTLWalker::get_g() const
{
    return m_g;
}

double  RTLWalker::get_v() const
{
    return m_v;
}

void RTLWalker::set_g(double g)
{
    m_g=g;
}

void RTLWalker::set_v(double v)
{
    m_v=v;
}
...
void RTLWalker::move(int verbose)
{
    ...
    double jump_length=gsl_ran_exponential(m_rng,m_g); //we first draw the time of the run
    //update all coordinates
    for(int i=0;i<m_d;i++){
        m_pos[i]+=m_v*jump_length*m_direction[i]; // move according to the jump drawn, so we multiply by the speed
    }
