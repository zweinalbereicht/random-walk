...
GaussianWalker::GaussianWalker()
    : m_name("a simple isotropic gaussian walker"), m_pos(0.0, 0.0),
      m_direction(0.0, 0.0), m_max(0.0), m_lifetime(0.0), m_seed(1), m_d(2) {
  for (int i = 0; i < 2; i++) {
    m_direction.push_back(0.0);
    m_pos.push_back(0.0);
  }
  m_rng =
      gsl_rng_alloc(gsl_rng_mt19937); // on prend le mersenne un peu par default
  gsl_rng_set(m_rng, m_seed);
}

GaussianWalker::GaussianWalker(string name, int d, const pybind11::list &pos,
                               int seed)
    : m_name(name), m_seed(seed), m_lifetime(0.0), m_d(d) {
  // on essaye tant bien que mal d'extraire cette merde
  for (int i = 0; i < d; i++)
    m_pos.push_back(pos[i].cast<double>()); // attention ici on cast en double
  for (int i = 0; i < d; i++)
    m_direction.push_back(
        0.0); // initialement la direction vaut 0 (pas de direction privilégiée)
  m_max = euclidian_distance(m_pos);
  m_rng = gsl_rng_alloc(gsl_rng_mt19937);
  gsl_rng_set(m_rng, seed);
}

GaussianWalker::~GaussianWalker() { gsl_rng_free(m_rng); }

void GaussianWalker::print_details() const {
...
double GaussianWalker::get_max() const { return m_max; }

double GaussianWalker::get_lifetime() const { return m_lifetime; }
...
void GaussianWalker::set_random_pos(const pybind11::list &dimensions) {
  for (int i = 0; i < m_pos.size(); i++)
    m_pos[i] = dimensions[i].cast<double>() *
               gsl_rng_uniform(m_rng); // uniform initial position in a box
}

void GaussianWalker::set_max(double max) { m_max = max; }

void GaussianWalker::set_lifetime(double lifetime) { m_lifetime = lifetime; }
...
void GaussianWalker::set_coord(int c, double x) { m_pos[c] = x; }

// the basic move function
void GaussianWalker::move(int verbose) {
  // start by updating the direction on the unit sphere
  sphere_direction(m_d, m_direction, m_rng);
  // draw a random length
  double jump_length =
      gsl_ran_gaussian(m_rng, 1.0); // just doing a gaussian step here for now
  // update all coordinates
  for (int i = 0; i < m_d; i++) {
    m_pos[i] +=
        jump_length * m_direction[i]; // move according to the jump drawn
  }
...
void GaussianWalker::move_bounded(const pybind11::list &dimensions,
                                  int verbose) {
  // start by updating the direction on the unit sphere
  sphere_direction(m_d, m_direction, m_rng);
  // draw a random length
  ...
  for (int i = 0; i < m_d; i++) {
    m_pos[i] +=
        fmod(jump_length * m_direction[i],
             dimensions[i].cast<double>()); // move according to the jump drawn
  }
  m_lifetime += 1; // for now, in the case of instantaneous jumps
  double dist = euclidian_distance(m_pos);
...
// other funtions
// these move til death are gonna lead to infinite buckles, we will need to
// change them
void GaussianWalker::move_til_death(int verbose) {
  while (isAlive()) {
    move(verbose);
  }
}

void GaussianWalker::move_til_death_bounded(
...
bool GaussianWalker::isAlive() const {
  return (euclidian_distance(m_pos) >=
          0); // For now this function is not working
}

void GaussianWalker::move_fixed_time(
    long time) // performs a certain number of steps --> the true time evolution
               // is different
{
...
void sphere_direction(const int d, std::vector<double> &dir, gsl_rng *rng) {
  double S = 0.0;
  for (int i = 0; i < d; i++) {
