...
py::class_<GammaWalker,ContinuousWalker>(handle, "GammaWalker")
    ...
    .def(py::init<double,double>()
            ,"takes entry scale(double), exponent(double) as defined in the GSL library. Beware, this is not necessarily the way we usually define it (wrt to the scale paramter for instance)")
